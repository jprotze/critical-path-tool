...
extern int myProcId;
extern bool useMpi;
extern ompt_finalize_tool_t ompt_finalize_tool;
...
struct SYNC_CLOCK {
  ...
  SYNC_CLOCK(double _useful_computation, double _mpi_start_time)
      : useful_computation_critical(_useful_computation), outsidempi_proc(_mpi_start_time), outsidempi_thread(_mpi_start_time), outsidempi_critical(_mpi_start_time) {}
  SYNC_CLOCK() {}
  void OmpHBefore() ;
  void OmpHAfter() ;
  void Print(const char* prefix1, const char* prefix2=""){printf("%s %s: ucp=%lf, uct=%lf, ucc=%lf, omp=%lf, omt=%lf, omc=%lf, oop=%lf, oot=%lf, ooc=%lf, oocno=%lf\n", 
             prefix1, prefix2, useful_computation_proc.load(), useful_computation_thread.load(), 
             useful_computation_critical.load(), outsidempi_proc.load(), 
             outsidempi_thread.load(), outsidempi_critical.load(), 
             outsideomp_proc.load(), outsideomp_thread.load(), outsideomp_critical.load(), outsideomp_critical_nooffset.load());}
};
...
//  bool stopped_mpi_clock{false};
  ...
  THREAD_CLOCK(int threadid,
               double _useful_computation, bool _openmp_thread = false)
      : SYNC_CLOCK(_useful_computation, (!analysis_flags->running)?0:-getTime()), thread_id(threadid),
        openmp_thread(_openmp_thread), stopped_mpi_clock(!analysis_flags->running) {}
  THREAD_CLOCK() {}

  void Stop(ClockContext cc=OMP, const char* loc=NULL) {
    if (cc!=OMP_ONLY && !analysis_flags->running)
      return;
    Stop(getTime(), cc, loc);
  }

  void Stop(double time, ClockContext cc=OMP, const char* loc=NULL) {
    ...
    if (cc!=MPI && cc!=MPI_ONLY) {
      ...
      atomic_add(outsideomp_critical_nooffset, time);
  ...
  void Start(ClockContext cc=OMP, const char* loc=NULL) {
    if (!analysis_flags->running)
      return;
    Start(-getTime(), cc, loc);
  }

  void Start(double time, ClockContext cc=OMP, const char* loc=NULL) {
...
//void stopClock(THREAD_CLOCK *thread_clock, bool mpi = false){thread_clock->Stop(mpi?MPI:OMP);}
//void stopOmpClock(THREAD_CLOCK *thread_clock){thread_clock->Stop(OMP_ONLY);}
//void startClock(THREAD_CLOCK *thread_clock, bool mpi = false){thread_clock->Start(mpi?MPI:OMP);}
#define stopClock(t) (t)->Stop(OMP, __func__)
#define startClock(t) (t)->Start(OMP, __func__)
#define stopMpiClock(t) (t)->Stop(MPI, __func__)
#define startMpiClock(t) (t)->Start(MPI, __func__)
#define stopOmpClock(t) (t)->Stop(OMP_ONLY, __func__)
void resetMpiClock(THREAD_CLOCK *thread_clock);

void startTool(bool toolControl=true, ClockContext cc=ALL);
void stopTool();

//void OmpHappensBefore(SYNC_CLOCK *cv);
//void OmpHappensAfter(SYNC_CLOCK *cv);
